...
<title>JqueryImgareaSelect--图片旋转/截取插件</title>
...
<link rel="stylesheet" href="<%=request.getContextPath()%>/ext/operamasks-ui/css/wzms/om-default.css" />
<script type="text/javascript" src="<%=request.getContextPath()%>/ext/operamasks-ui/js/operamasks-ui.min.js"></script>

<style type="text/css">
.demo{width:720px;height:400px;margin:20px auto 0 auto;}
.portrait_left{float:left;height:auto;width:306px;}
#picture{border:1px solid #000000;overflow:hidden;position:relative;margin:0 auto;}
...
<body>
	...
	<input type="hidden" name="obj.zp" value=""/>

	<div class="portrait_left">
	
		<div id="picture"><img id="avatar" alt="请上传头像" height="300" width="300" src="test/Koala_cropped.jpg"/></div>
		...
		<button class="baseinf_but1" onclick="submit_avatar();">确定</button></div>
	</div>
...
$(document).ready(function (){
	...
	$('#file_upload').omFileUpload({
	    action : '<%=request.getContextPath()%>/1001',
	    method:'GET',
	    actionData : {'type':'testimg', 'zipwidth':$("#avatar").attr("width"), 'zipheight':$("#avatar").attr("width")},
	    swf : '<%=request.getContextPath()%>/ext/operamasks-ui/swf/om-fileupload.swf',
	    autoUpload : true,
	    fileExt : '*.jpg;*.jpeg;*.png;*.gif',
	    fileDesc : 'Image Files',
	    sizeLimit : 1024000,
	    buttonText : '上传照片',
	    // buttonImg: '<%=request.getContextPath()%>/images/upload-btn.jpg',
	    onComplete : function(ID,fileObj,response,data,event){
	    	var jsonData = eval("("+response+")");
	    	$("#avatar").attr("src","<%=request.getContextPath()%>/"+jsonData.d.url);
	    	$("#avatar1").attr("src","<%=request.getContextPath()%>/"+jsonData.d.url);
	    	$("#avatar2").attr("src","<%=request.getContextPath()%>/"+jsonData.d.url);
	    	$("[name='obj.zp']").val(jsonData.d.url);
	    },
